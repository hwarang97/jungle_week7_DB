...
| 기준 | 의미 | 이번 프로젝트 적용 |
|---|---|---|
| 자주 찾는가 | 실제 조회 조건에 자주 등장하는 컬럼인지 봅니다. | `id` 조회가 핵심 패턴입니다. |
| 잘 구분하는가 | 값이 row를 충분히 잘 나누는지 봅니다. | `id` 는 row를 잘 구분합니다. |
| 정렬/범위/조인에 쓰이는가 | 범용적인 인덱스 구조인지 봅니다. | B+ Tree는 `=`과 범위 탐색 모두 확장성이 있습니다. |
| 유지 비용보다 이득이 큰가 | 읽기 이득이 쓰기/관리 비용보다 큰지 봅니다. | 읽기 최적화를 위해 `id` 인덱스를 우선 적용했습니다. |

| 트레이드오프 | 설명 |
|---|---|
| 읽기 장점 | `WHERE id = ?` 는 전체 row를 끝까지 읽지 않고 빠르게 탐색할 수 있습니다. |
| 규모 효과 | 데이터가 많아질수록 선형 탐색과의 차이가 더 커집니다. |
| 쓰기 비용 | `INSERT`, `UPDATE`, `DELETE` 때는 인덱스도 함께 갱신해야 합니다. |
...
이 프로젝트는 `WHERE id = ?` 경로를 실제 SQL 처리기에 연결하는 것이 핵심이기 때문에, 범용성이 높은 B+ Tree를 선택했습니다.
